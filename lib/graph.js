...
function GraphRequest(graph, accessToken, method, url, postData, callback) {
  ...
  this.options = extend({}, graph.requestOptions);
...
GraphRequest.prototype.cleanUrl = function (url) {
  ...
  if (this.accessToken && this.graph.appSecret && url.indexOf('appsecret_proof') === -1) {
...
Graph.prototype.get = function (url, params, callback) {
  ...
  var accessToken = params.access_token || this.accessToken || undefined;

  return new GraphRequest(this, accessToken, 'GET', url, callback);
};
...
Graph.prototype.post = function (url, params, postData, callback) {
  if (typeof url !== 'string') {
    return callback({ message: 'GraphRequest api url must be a string' }, null);
  }
  ...
  return new GraphRequest(this, accessToken, 'POST', url, postData, callback);
};
...
Graph.prototype.del = function (url, params, callback) {
...
Graph.prototype.search = function (options, params, callback) {
  options = options || {};
 ...
 * @param {function} callback
 */
Graph.prototype.fql = function (query, params, callback) {
